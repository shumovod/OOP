...
void RenderTerminal::print_logger() const {
    std::cout << "Do you want to record logs? (Y/N): " << std::endl;
}

void RenderTerminal::print_choose_logger() const {
    std::cout << "Where do you want to write logs?" << std::endl
              << "(1 - terminal, 2 - file, 3 - both): " << std::endl;
}
...
void RenderTerminal::clear() const {
    system("cls");
}